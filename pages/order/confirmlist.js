...
$(document).ready(function() {
	...
	$("a#orderViewLink").on("click", function() {
		if (orderid == 0) {
			...
			$("div#order-confirm-dialog").load("pages/order/orderDetail.html", function() {
				$("#pay-button").text("确认收货");
				function confirmClick(){
					var orderid = $("#detail-order-id").html();
					$.ajax({
						url: "http://localhost:80/order/confirm",
						type: "GET",
						dataType: "json",
						data: {id: orderid},
						headers: {
