...
$(document).ready(function() {
	...
	function loadAndShow() {
		$.ajax({
			url: api + "/commodity/sale/shopper",
			type: "GET",
			dataType: "json",
			data: {
				page: page,
	...
	$("a#commodityViewLink").on("click", function() {
		if (commodityid == 0) {
			...
			$("div#commodity-mysale-dialog").html("");
			$("div#commodity-mysale-dialog").load("pages/commoditySale/commodityDetail.html", function() {
				$.ajax({
	...
	//删除该物品
	$("a#commodity-mysale-delete").on("click", function() {
			if (commodityid == 0) {
				alert("请点击条目选择要删除的商品");
			} else {
				$.ajax({
	...
	//修改物品信息
	$("a#commodity-mysale-update").on("click",function(){
		if(commodityid==0){
			alert("请选择要修改的物品");
		}else{
			alert("加载update"+commodityid);
			$("div#commodity-update-dialog").html("");
			$("div#commodity-update-dialog").load("pages/commoditySale/commodityUpdate.html",function(){
				// 设置表单提交地址
				$("form#commodityUpdateForm").attr("action",api+"/commodity/sale/update");
				function labelShow() {
					$.ajax({
				...
				$.ajax({
					...
					success: function(resultDate) {
						if (resultDate.code == 200) {
							...
							$("input#exampleInputShabby-update").attr("placeholder",resultDate.data.shabby);
							$("input#exampleInputIntroduce-update").attr("placeholder",resultDate.data.introduce);
						} else
							alert("回显失败");
					},
					error: function(xhr) {
