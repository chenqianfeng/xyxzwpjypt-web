...
$(document).ready(function() {
	...
	function loadAndShow() {
		$.ajax({
			url: api + "/commodity/sale/view/list",
			type: "GET",
			dataType: "json",
			data: {
				status: 1,
			...
			success: function(resultDate) {
				if (resultDate.code == 200) {
					...
					$("table#commodity-hot-table tbody").html("");
					$.each(resultDate.data, function(index, dm) {
						var tr = "<tr id='" + dm.id + "'><td>" + dm.name + "</td><td>" + dm.price +
							...
							dm.introduce + "</td></tr>";
						$("table#commodity-hot-table tbody").append(tr);
					});
					$("table#commodity-hot-table tbody tr").on("click", function() {
						commodityid = $(this).attr("id");
						$("table#commodity-hot-table tbody tr").css("background-color", "#FFFFFF");
	...
	$("a#commodityViewLink").on("click", function() {
		if (commodityid == 0) {
			...
			$("div#commodity-hot-dialog").load("pages/commoditySale/commodityDetail.html", function() {
				$.ajax({
