...
$(document).ready(function() {
	...
	function loadAndShow() {
		...
		$.ajax({
			url: api + "/commodity/sale/condition",
			type: "GET",
			dataType: "json",
			data: {
				key: $("#searchInputkey").val(),
				min:$("#searchInputmin").val(),
				max:$("#searchInputmax").val(),
				label:$("#searchInputlabel").val(),
				page: page,
				row: rows
			},
			headers: {
				...
				if (resultDate.code == 200) {
					...
					$.each(resultDate.data, function(index, dm) {
						var tr = "<tr id='" + dm.id + "'><td>" + dm.name + "</td><td>" + dm
							.price +
							"</td><td>" + dm.shabby + "</td><td>" + dm.releaseDate +
							"</td><td>" +
							dm.introduce + "</td></tr>";
	...
	$("button#searchSaleInputSubmit").on("click", function() {
		if(($("#searchInputkey").val()==null||$("#searchInputkey").val()=="")
			...
			&&($("#searchInputlabel").val()==null||$("#searchInputlabel").val()=="")
	...
	$("a#commodityViewLink").on("click", function() {
		if (commodityid == 0) {
			...
			$("div#commodity-search-dialog").load("pages/commoditySale/commodityDetail.html", function() {
				$.ajax({
					...
					success: function(resultDate) {
						if (resultDate.code == 200) {
							...
							$.each(resultDate.data.words, function(index, dm) {
								var tr2 = "<label id='" + dm.id + "'>" +
									"用户id：" +
									dm.commodityId + " 时间：" + dm.wordsTime +
									" 留言：" + dm.words + "</label><br/>";
								$("div#detail-div div[id='words']").append(
									tr2);
							});
